...
async function validateSelectedMojangAccount(){
    ...
    // Se o token for 'offline_token', ignoramos a validação externa
    if(current.accessToken === 'offline_token') {
        log.info('Validando conta offline (Ignorando servidores da Mojang).')
        return true
    }
 ...
 * otherwise false.
 *//**
 * Adiciona uma conta Offline (Pirata).
 * * @param {string} username O nick escolhido pelo jogador.
 * @returns {Promise.<Object>} Objeto da conta autenticada localmente.
 */
exports.addOfflineAccount = async function(username) {
    try {
        // Criamos um UUID baseado no nome (padrão offline)
        const uuid = 'offline-' + username.toLowerCase().replace(/[^a-z0-9]/g, '');
        
        // Adicionamos ao ConfigManager usando o tipo mojang para estrutura compatível
        const ret = ConfigManager.addMojangAuthAccount(uuid, 'offline_token', username, username)
        
        ConfigManager.save()
        log.info(`Conta Offline adicionada: ${username}`)
        return ret
    } catch (err) {
        log.error('Erro ao adicionar conta offline', err)
        return Promise.reject({
            title: "Erro ao entrar",
            desc: "Não foi possível criar sua conta offline."
        })
    }
}
